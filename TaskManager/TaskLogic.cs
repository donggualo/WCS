    ...
    public class TaskLogic
    {
        ...
        public void Task_InInitial(WCS_COMMAND_V command)
        {
            try
            {
                ...
                // 运输车到摆渡车对接点
                DataControl._mTaskTools.CreateCustomItem(command.WCS_NO, ItemId.运输车复位1, "",
                    loc == loc1 ? command.TASK_UID_1 : command.TASK_UID_2, DataControl._mStools.GetValueByKey("StandbyR1"), ItemStatus.不可执行);
                ...
                // 行车到运输车对接取货点
                if (ABC_1 == ABC_2 || (string.IsNullOrEmpty(ABC_1) || string.IsNullOrEmpty(ABC_2)))
                {
                    // 当所有入库坐标为同一行车分区，则只生成一笔行车定位任务
                    String ABCloc = DataControl._mTaskTools.GetABCTrackLoc(loc == loc1 ? command.LOC_TO_1 : command.LOC_TO_2);     //获取对应行车位置
                    DataControl._mTaskTools.CreateCustomItem(command.WCS_NO, ItemId.行车轨道定位,
                        string.IsNullOrEmpty(ABC_1) ? ABC_2 : ABC_1, string.IsNullOrEmpty(ABC_1) ? command.TASK_UID_2 : command.TASK_UID_1, ABCloc, ItemStatus.请求执行);
                }
                else
                {
                    if (loc == loc1)
                    {
                        string ABCloc_1 = DataControl._mTaskTools.GetABCTrackLoc(command.LOC_TO_1);
                        string ABCloc_2 = DataControl._mTaskTools.GetABCTrackLoc(command.LOC_TO_2);
                        DataControl._mTaskTools.CreateCustomItem(command.WCS_NO, ItemId.行车轨道定位, ABC_1, command.TASK_UID_1, ABCloc_1, ItemStatus.请求执行);
                        DataControl._mTaskTools.CreateCustomItem(command.WCS_NO, ItemId.行车轨道定位, ABC_2, command.TASK_UID_2, ABCloc_2, ItemStatus.请求执行);
                    }
                    else
                    {
                        string ABCloc_2 = DataControl._mTaskTools.GetABCTrackLoc(command.LOC_TO_1);
                        string ABCloc_1 = DataControl._mTaskTools.GetABCTrackLoc(command.LOC_TO_2);
                        DataControl._mTaskTools.CreateCustomItem(command.WCS_NO, ItemId.行车轨道定位, ABC_1, command.TASK_UID_2, ABCloc_2, ItemStatus.请求执行);
                        DataControl._mTaskTools.CreateCustomItem(command.WCS_NO, ItemId.行车轨道定位, ABC_2, command.TASK_UID_1, ABCloc_1, ItemStatus.请求执行);
                    }
                }
        ...
        public void CreateTask_RGV_ABC(WCS_TASK_ITEM item)
        {
            ...
            {
                // 查看运输车是否在行车对接待命点
                String sql_R = String.Format(@"select ID, WCS_NO, ITEM_ID, DEVICE from WCS_TASK_ITEM where STATUS = '{2}' and WCS_NO = '{0}' 
                    and ITEM_ID = '{1}' and LOC_FROM = '{3}'", item.WCS_NO, ItemId.运输车定位, ItemStatus.交接中, item.LOC_FROM);
                DataTable dtitem_R = DataControl._mMySql.SelectAll(sql_R);
                if (DataControl._mStools.IsNoData(dtitem_R))
                {
                    return;
                ...
                switch (item.WCS_NO.Substring(0, 1))
                {
                    case "I":   // 入库  运输车 (货物)==> 行车 
                        // 行车取货
                        DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.行车取货, item.DEVICE, item.LOC_FROM, item.LOC_TO, ItemStatus.请求执行);
                        break;
                    case "O":   // 出库  行车 (货物)==> 运输车 
                        // 行车放货
                        DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.行车放货, item.DEVICE, item.LOC_FROM, item.LOC_TO, ItemStatus.请求执行);
                        break;
                    default:
                        break;
                }
        ...
        public void Run_TaskContinued()
        {
            try
            {
                ...
                string sql = string.Format(@"select * from WCS_TASK_ITEM where (WCS_NO,ID) in 
(select WCS_NO, MAX(ID) from WCS_TASK_ITEM where ITEM_ID not in ('{0}','{1}') and LEFT(ITEM_ID,2) = '11' group by WCS_NO) 
  union
 select * from wcs_task_item where (WCS_NO,ID) in (select WCS_NO, MAX(ID) from WCS_TASK_ITEM where ITEM_ID in ('{0}','{1}') group by WCS_NO)", ItemId.行车取货, ItemId.行车放货);
                DataTable dtlast = DataControl._mMySql.SelectAll(@"sql");
            ...
            try
            {
                ...
                switch (item.ITEM_ID)   //根据最后的设备指令，可得货物已在流程中该设备对接的下一设备处
                {
                    case ItemId.固定辊台正向: //目的设备为对接的摆渡车，可直接加以分配
                        ...
                        // 存在摆渡车对接运输车定位任务则跳过
                        taskcount = DataControl._mMySql.GetCount("wcs_task_item", String.Format(@"WCS_NO = '{0}' and ID > {1} and ITEM_ID = '{2}'",
                            item.WCS_NO, item.ID, ItemId.摆渡车定位运输车对接));
                        if (taskcount != 0)
                        {
                            break;
                        }
                        // 判断对应的摆渡车是否完成
                        if (DataControl._mTaskTools.GetInTaskStatus(item.ID) != ItemStatus.完成任务)
                        {
                            // 未完成则跳过
                            break;
                        }
                        // 可断定货物需移至运输车
                        // 当属于中间最接点固定辊台时，直接等待对接
                        if (item.DEVICE == DataControl._mTaskTools.GetFRTByARFLoc(DataControl._mStools.GetValueByKey("StandbyAR")))
                        {
                            // 生成摆渡车任务
                            DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.摆渡车定位运输车对接, item.LOC_TO, "", AR, ItemStatus.交接中);
                        }
                        else
                        {
                            // 生成摆渡车任务
                    ...
                    case ItemId.摆渡车正向:  //目的设备为对接的运输车，可直接加以分配
                        ...
                        // 存在任务则跳过
                        taskcount = DataControl._mMySql.GetCount("wcs_task_item", String.Format(@"WCS_NO = '{0}' and ID > {1} and ITEM_ID in ('{2}','{3}')",
                            item.WCS_NO, item.ID, ItemId.运输车对接定位, ItemId.运输车定位));
                        if (taskcount != 0)
                        {
                            break;
                        }
                        ...
                        string task = loc == loc_1 ? command.TASK_UID_1 : command.TASK_UID_2;
                        if (loc == "NG")
                        {
                            //不能没有货物目的位置
                            break;
                        ...
                        {
                            // 生成运输车[内]复位任务,待分配设备
                            DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车复位2, "", task, R, ItemStatus.不可执行);
                            // 生成运输车[外]对接位任务
                            DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车对接定位, item.LOC_TO, task, RR, ItemStatus.请求执行);
                        }
                        else
                        {
                            // 生成运输车[外]定位任务
                            DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车定位, item.LOC_TO, task, loc, ItemStatus.请求执行);
                        }
                        #endregion
                    ...
                    case ItemId.运输车正向:  //目的设备为对接的运输车，可直接加以分配
                        ...
                        // 存在任务则跳过
                        taskcount = DataControl._mMySql.GetCount("wcs_task_item", String.Format(@"WCS_NO = '{0}' and ID > {1} and ITEM_ID = '{2}'",
                            item.WCS_NO, item.ID, ItemId.运输车定位));
                        if (taskcount != 0)
                        {
                            break;
                        }
                        ...
                        // 生成运输车定位任务
                        DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车定位, item.LOC_TO,
                            loc == loc_1 ? command.TASK_UID_1 : command.TASK_UID_2, loc, ItemStatus.请求执行);
                        #endregion

                        break;
                    case ItemId.行车取货:
                        #region 将运输车移至行车对接位置 && 行车库存定位
                        // 无行车库存定位任务
                        taskcount = DataControl._mMySql.GetCount("wcs_task_item", String.Format(@"WCS_NO = '{0}' and ID > {1} and ITEM_ID = '{2}' and DEVICE = '{3}'",
                            item.WCS_NO, item.ID, ItemId.行车库存定位, item.DEVICE));
                        if (taskcount == 0)
                        {
                            // 生成行车库存定位任务
                            string locABC = DataControl._mTaskTools.GetABCStockLoc(item.LOC_FROM == command.TASK_UID_1 ? command.LOC_TO_1 : command.LOC_TO_2);
                            DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.行车库存定位, item.DEVICE, item.LOC_FROM, locABC, ItemStatus.请求执行);
                        }
                        ...
                        if (!DataControl._mTaskTools.IsRGVtaskOK(item.WCS_NO))
                        {
                            ...
                            {
                                taskID = command.TASK_UID_1;
                                loc_Y = loc_1;
                                loc_N = loc_2;
                            }
                            else if (_RGV.GoodsStatus() == RGV.GoodsYes2) // 辊台②有货
                            {
                                taskID = command.TASK_UID_2;
                                loc_Y = loc_2;
                                loc_N = loc_1;
                            }
                            else
                            ...
                            if (isGoodsRGV)
                            {
                                ...
                                {
                                    // 生成运输车[内]复位任务,待分配设备
                                    DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车复位2, "", taskID, R, ItemStatus.不可执行);
                                    // 生成运输车[外]对接位任务
                                    DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车对接定位, rgv, taskID, RR, ItemStatus.请求执行);
                                }
                                else
                                {
                                    // 生成运输车 定位任务
                                    DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.运输车定位, rgv, taskID, loc_Y, ItemStatus.请求执行);
                                }
                            }
                        }
                    ...
                    case ItemId.行车放货:
                        #region 行车轨道定位
                        // 存在2笔行车定位任务则跳过
                        taskcount = DataControl._mMySql.GetCount("wcs_task_item", String.Format(@"WCS_NO = '{0}' and ITEM_ID = '{1}'",
                            item.WCS_NO, ItemId.行车轨道定位));
                        if (taskcount == 2)
                        {
                            break;
                        }
                        // 是否有未完成的任务目标点
                        if ((string.IsNullOrEmpty(command.TASK_UID_1) || string.IsNullOrEmpty(command.TASK_UID_2)) ||
                            (command.SITE_1 == TaskSite.完成 && command.SITE_2 == TaskSite.完成))
                        {
                            break;
                        }
                        ...
                        // 生成行车轨道定位任务
                        DataControl._mTaskTools.CreateCustomItem(item.WCS_NO, ItemId.行车轨道定位, item.DEVICE,
                            item.LOC_FROM == command.TASK_UID_1 ? command.TASK_UID_1 : command.TASK_UID_2, ABCloc, ItemStatus.请求执行);
                        #endregion
